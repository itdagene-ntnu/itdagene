...
def user_detail(request, pk):
    try:
        if not request.user.is_staff:
            person = User.objects.filter(Q(is_staff=True) | Q(id=request.user.id)).filter(
                is_active=True, year=request.user.year
            ).get(pk=pk)
        else:
...
def user_edit_password(request, pk):
    ...
    return render(
        request, 'users/edit_password.html', {
            'person': person,
            'form': form,
...
def user_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(reverse('itdagene.users.user_detail', kwargs={'pk': person.pk}))
    else:
        form = UserCreateForm()
...
def send_welcome_email(request, pk):
    ...
    return render(
        request, 'users/send_welcome_email.html', {
            'person': user,
...
def vcard(request, pk):
    try:
        if not request.user.is_staff:
            person = User.objects.filter(Q(is_staff=True)
                                         | Q(id=request.user.id)).filter(is_active=True).get(pk=pk)
        else:
            person = User.objects.filter(is_active=True).order_by('username').get(pk=pk)
    except Exception:
        raise Http404
