...
class Company(BaseModel):
    ...
    logo_vector = models.FileField(upload_to="company_logos/",
                                   null=True,
                                   blank=True,
    ...
    def get_first_day(cls):
        return (
            cls.get_signed_with_packages().select_related("package").filter(
                package__has_stand_first_day=True).exclude(
                    logo="").order_by("package__max"))

    @classmethod
    def get_last_day(cls):
        return (
            cls.get_signed_with_packages().select_related("package").filter(
                package__has_stand_last_day=True).exclude(
                    logo="").order_by("package__max"))
...
class CompanyContact(BaseModel):
    ...
    current = models.BooleanField(default=False,
                                  verbose_name=_("Current contact"))
