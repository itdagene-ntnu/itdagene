...
def view(request, id):
    ...
    return render(request, 'adm/users/view.html', {'t_user': user})
...
def edit(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(reverse('itdagene.app.admin.views.users.view', args=[user.pk]))
    return render(request, 'adm/users/edit.html', {'t_user': user, 'form': form})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if cache.get('pref'):
                ...
                cache.set('pref', pref)
            try:
                user = User(username=username, email=username + '@stud.ntnu.no',
