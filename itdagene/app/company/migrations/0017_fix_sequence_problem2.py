...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if settings.POSTGRESQL:
            highest_number = db.execute('select id from company_comment order by id desc limit 1;')[0][0]
            db.execute('alter sequence company_comment_id_seq restart with %s;' % (highest_number + 1))
