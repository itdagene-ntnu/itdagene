...
def frontpage(request):
    ...
    if request.user.is_staff:
        ...
        assigned_you_issues = Issue.objects.filter(is_solved=False,
                                                   assigned_user=request.user).count()
        overdue_issues = Issue.objects.filter(is_solved=False, deadline__lte=timezone.now()).count()
        ...
        company_comments = Comment.objects.filter(content_type=company_type
                                                  ).order_by('-date')[:8].select_related(
                                                      'user',
                                                      'content_type',
