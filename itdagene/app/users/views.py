...
def user_list(request):
    if not request.user.is_staff:
        persons = User.objects.filter(Q(is_staff=True) | Q(
            id=request.user.id)).filter(is_active=True,
...
def user_detail(request, pk):
    try:
        if not request.user.is_staff:
            person = User.objects.filter(Q(is_staff=True) | Q(
                id=request.user.id)).filter(is_active=True,
                                            year=request.user.year).get(pk=pk)
        else:
            person = User.objects.filter(is_active=True).order_by(
                'username').get(pk=pk)
    except:
        raise Http404
...
def user_edit(request, pk):
    ...
    if request.method == 'POST':
        if request.user.is_superuser:
            ...
            form = SimpleUserEditForm(request.POST, request.FILES,
                                      instance=person)
        if form.is_valid():
            person = form.save()
...
def send_welcome_email(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect(reverse('itdagene.app.users.views.user_detail',
                                kwargs={'pk': user.pk}))
    return render(request, 'users/send_welcome_email.html', {
        'person': user,
...
def vcard(request, pk):
    try:
        if not request.user.is_staff:
            person = User.objects.filter(Q(is_staff=True) | Q(
                id=request.user.id)).filter(is_active=True).get(pk=pk)
        else:
            person = User.objects.filter(is_active=True).order_by(
                'username').get(pk=pk)
    except:
