...
class TestJoblistings(TestCase):
    ...
    def test_inactive_joblisting_is_not_in_connection(self):
        Joblisting.objects.create(company=self.company,
    ...
    def test_inactive_joblisting_is_node(self):
        ...
        joblisting = Joblisting.objects.create(company=self.company,
                                               deadline=timezone.now() -
                                               timedelta(days=1))
        global_id = to_global_id("Joblisting", joblisting.pk)
        executed = self.client.execute(self.node_query,
                                       variable_values={"id": global_id})

        self.assertIsNone(executed.get("errors"))
        self.assertIsNotNone(executed["data"]["node"])

        self.assertEqual(executed["data"]["node"], {
            "id": global_id,
    ...
    def test_only_active_is_in_search(self):
        ...
        executed = self.client.execute(self.search_query,
                                       variable_values={"query": title})
